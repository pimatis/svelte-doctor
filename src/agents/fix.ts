...
const SUPPORTED_AGENT_IDS =
	"cursor, amp, claude, codex, copilot, opencode, pi, gemini, qwen, aider, goose";
...
const cleanupPromptBundle = (bundle: { dir: string; path: string }): void => {
	try {
		...
		logger.dim(
			`  Could not remove prompt directory: ${error instanceof Error ? error.message : String(error)}`,
		);
	}
};
...
const spawnAgent = (
	...
	const stdio: [typeof stdinMode, typeof stdoutMode, "inherit"] = [
		stdinMode,
		stdoutMode,
		"inherit",
	];
...
const printPromptFallback = (promptPath: string, exitCode: number): void => {
	...
	logger.break();
	logger.dim(
		"  Paste the file contents into your preferred AI agent manually.",
	);
};
...
export const runCommand = (
	...
	new Promise((resolve) => {
		...
		child.on("error", () =>
			resolve({ ok: false, status: "missing-binary", code: null }),
		);
	});
...
const logCommandFailure = (label: string, result: CommandResult): void => {
	...
	logger.error(
		`  Command failed while running ${label}${typeof result.code === "number" ? ` (exit ${result.code})` : ""}.`,
	);
};
...
export const verifyScripts = async (
	...
	const packageManager =
		options.packageManager ?? resolvePackageManager(directory);
...
export const runFix = async (
	...
	const beforeWarnings = diagnostics.filter(
		(d) => d.severity === "warning",
	).length;
	const maxFiles = Math.max(
		1,
		Math.min(
			options.maxFiles ?? DEFAULT_FIX_MAX_FILES,
			diagnostics.length || DEFAULT_FIX_MAX_FILES,
		),
	);
	...
	const pickAgent = (): AgentInfo | null => {
		if (options.agentOverride) {
			const forced = agents.find(
				(a) => (a.id ?? a.command) === options.agentOverride,
			);
			if (!forced) {
				logger.error(
					`  Unknown agent: ${options.agentOverride}. Available: ${SUPPORTED_AGENT_IDS}`,
				);
				return null;
			}
			if (!forced.available) {
	...
	logger.log(
		`  Using ${highlighter.info(agent.name)} to fix ${highlighter.warn(String(Math.min(diagnostics.length, maxFiles)))} issues...`,
	);
	logger.dim(
		`  Agent mode: ${options.unsafeAgentExec ? highlighter.warn("unsafe opt-in") : highlighter.success("safe by default")}`,
	);
	if (agent.formatStreamingOutput) {
		logger.dim(
			"  Large fix sets may take several minutes. Streaming output below...",
		);
	}
	logger.break();
	...
	const code = await spawnAgent(
		...
		directory,
		prompt,
		options.unsafeAgentExec ? "unsafe" : "safe",
	);
	...
	const verification = await verifyFixResult(
		directory,
		beforeErrors,
		beforeWarnings,
		verifyLevel,
...
const verifyFixResult = async (
	...
	try {
		...
		const afterErrors = result.diagnostics.filter(
			(d) => d.severity === "error",
		).length;
		const afterWarnings = result.diagnostics.filter(
			(d) => d.severity === "warning",
		).length;
		...
		if (afterWarnings < beforeWarnings) {
			logger.success(
				`  ✓ Warnings reduced: ${beforeWarnings} → ${afterWarnings}`,
			);
		}
		logger.success(`  ✓ Verification passed at ${verifyLevel} level.`);
		logger.break();
		logger.dim(
			`  Run ${highlighter.info("svelte-doctor check")} for full report.`,
		);
		logger.break();
