...
export const detectAgents = (): AgentInfo[] => [
  ...
  { name: "Claude Code", command: "claude", available: isCommandAvailable("claude") },
  { name: "Codex", command: "codex", available: isCommandAvailable("codex") },
];

export const getAvailableAgents = (): AgentInfo[] =>
  detectAgents().filter((a) => a.available);

// Pick the best available agent with priority amp then claude then codex.
export const getPreferredAgent = (): AgentInfo | null => {
  const available = getAvailableAgents();
  if (available.length === 0) return null;

  const priority = ["amp", "claude", "codex"];

  for (const cmd of priority) {
    const agent = available.find((a) => a.command === cmd);
