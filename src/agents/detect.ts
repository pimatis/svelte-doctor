...
export const getAgentAvailability = (): Record<string, boolean> =>
  Object.fromEntries(detectAgents().map((agent) => [agent.id ?? agent.command, agent.available]));
...
export const getPreferredAgent = (): AgentInfo | null => {
  ...
  const priority = ["cursor", "amp", "claude", "codex", "opencode", "pi", "gemini", "qwen", "aider", "goose"];
