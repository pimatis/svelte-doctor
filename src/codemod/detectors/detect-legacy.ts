...
const PATTERNS: Array<{ key: string; label: string; pattern: RegExp }> = [
  ...
  { key: "event-dispatcher", label: "createEventDispatcher", pattern: /\bcreateEventDispatcher\b/g },
  { key: "slot", label: "slots", pattern: /<\/?slot(?:\s|>|\/)/g },
  { key: "on-directive", label: "event directives", pattern: /\son:[A-Za-z]/g },
  { key: "lifecycle", label: "legacy lifecycle", pattern: /\b(?:onMount|onDestroy|beforeUpdate|afterUpdate)\b/g },
...
export const detectLegacy = (source: string): LegacyDetection[] =>
  PATTERNS.map((entry) => ({ key: entry.key, label: entry.label, count: [...source.matchAll(entry.pattern)].length }))
