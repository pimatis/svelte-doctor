...
export const eventDispatcherTransform: CodemodTransform = {
  ...
  run(source) {
    ...
    let changed = false;
    let nextScript = script.content.replace(
      ...
      (match, imports: string) => {
        ...
        if (remaining.length === 0)
          return "// TODO: createEventDispatcher removed, use callback props from $props()";
        return `import { ${remaining.join(", ")} } from "svelte";`;
      },
    );

    nextScript = nextScript.replace(
      /^([ \t]*)const\s+(\w+)\s*=\s*createEventDispatcher\s*\([^)]*\)\s*;?/gm,
      (_match, indent) => {
        changed = true;
