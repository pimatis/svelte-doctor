...
export const lifecycleTransform: CodemodTransform = {
  ...
  run(source) {
    ...
    const nextScript = script.content.replace(/import\s+\{([^}]*?)\}\s+from\s+["']svelte["'];?/g, (match, imports: string) => {
      ...
      changed = true;
      sawPreEffect = removed.some((part) => part.startsWith("beforeUpdate") || part.startsWith("afterUpdate"));
    ...
    return createResult(replaceInstanceScript(source, script, nextScript), "lifecycle", sawPreEffect ? "beforeUpdate/afterUpdate -> $effect.pre" : "lifecycle -> $effect");
  },
};
