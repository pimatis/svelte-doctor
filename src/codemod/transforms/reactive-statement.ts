...
const transformStatement = (statement: ts.LabeledStatement, ast: ts.SourceFile): string => {
  ...
  if (ts.isExpressionStatement(inner) && ts.isBinaryExpression(inner.expression)) {
    ...
    if (expression.operatorToken.kind === ts.SyntaxKind.EqualsToken && ts.isIdentifier(expression.left)) {
  ...
  const innerExpression = ts.isExpressionStatement(inner) ? unwrapParentheses(inner.expression) : null;
  if (innerExpression && ts.isBinaryExpression(innerExpression)) {
    ...
    if (expression.operatorToken.kind === ts.SyntaxKind.EqualsToken && ts.isObjectLiteralExpression(left)) {
  ...
  if (isSideEffectText(text)) return `${indent}$effect(() => { ${text.endsWith(";") ? text : `${text};`} });`;
...
export const reactiveStatementTransform: CodemodTransform = {
  ...
  run(source) {
    ...
    const edits = ast.statements
      ...
      .map((statement) => ({ start: skipLeadingNewlines(ast.text, statement.getFullStart()), end: statement.end, text: transformStatement(statement, ast) }));

    if (edits.length === 0) return createNoopResult(source);

    const nextScript = applyTextEdits(script.content, edits);
    return createResult(replaceInstanceScript(source, script, nextScript), "reactive-statement", "$: -> $derived/$effect");
  },
};
