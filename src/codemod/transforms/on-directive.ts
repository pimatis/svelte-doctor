...
import { createNoopResult, createResult, findMarkupExcludedRanges, isLikelyInsideString } from "../utils.js";
...
export const onDirectiveTransform: CodemodTransform = {
  ...
  run(source) {
    ...
    let changed = false;
    const next = source.replace(/\son:([A-Za-z][\w-]*)(\|[A-Za-z|]+)?(?=\s*=|[\s/>])/g, (match, eventName: string, modifiers: string | undefined, offset: number) => {
      if (isInsideRange(offset, excluded)) return match;
      if (isLikelyInsideString(source, offset)) return match;
      changed = true;
      if (modifiers) return ` on${eventName} /* TODO: modifiers ${modifiers} removed, handle manually */`;
      return ` on${eventName}`;
    });
