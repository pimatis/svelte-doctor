...
export const fixCommand = new Command("fix")
	...
	.option(
		"--errors-only",
		"fix only errors first (reduces cascade risk, run again for warnings)",
	)
	.option(
		"--unsafe-agent-exec",
		"allow agent-specific privileged execution flags (opt-in only)",
	)
	.option(
		"--dry-run-prompt",
	...
	.option(
		"--max-files <count>",
		"maximum diagnostics to include in a single agent batch",
		"50",
	)
	.addHelpText(
		"after",
	...
	.action(async (directory: string, flags: Record<string, unknown>) => {
		try {
			...
			if (flags.errorsOnly && diagnostics.length === 0) {
				logger.success(
					"  ✓ No errors to fix. Run without --errors-only to fix warnings.",
				);
				return;
			}
