...
`)
  .action(async (directory: string, flags: Record<string, unknown>) => {
    try {
      ...
      logger.break(); logger.log(`  ${highlighter.bold("svelte-doctor fix")} v${VERSION}`); logger.break();
      const result = await scan(resolvedDir, { quiet: true });
      const diagnostics = flags.errorsOnly ? result.diagnostics.filter((d) => d.severity === "error") : result.diagnostics;
      if (flags.errorsOnly && diagnostics.length === 0) { logger.success("  ✓ No errors to fix. Run without --errors-only to fix warnings."); return; }
      const maxFiles = parsePositiveInt(flags.maxFiles as string, "max files");
      const fixResult = await runFix(resolvedDir, diagnostics, { agentOverride: flags.agent as string, unsafeAgentExec: (flags.unsafeAgentExec as boolean) ?? false, dryRunPrompt: (flags.dryRunPrompt as boolean) ?? false, verifyLevel: flags.verifyLevel as VerificationLevel, maxFiles: maxFiles > 0 ? maxFiles : 50 });
      if (fixResult?.errorsIncreased || fixResult?.verificationPassed === false) process.exitCode = 1;
