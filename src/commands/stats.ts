...
export const statsCommand = new Command("stats")
  ...
  .action(async (directory: string, flags: { json?: boolean; top: string }) => {
    try {
      ...
      logger.break(); logger.log(`  ${highlighter.bold("svelte-doctor stats")} v${VERSION}`); logger.break();
      ...
      logger.break();
      if (result.categories.length > 0) { logger.log(`  ${highlighter.bold("Categories:")}`); for (const c of result.categories) logger.log(`    ${sanitize(c.category)}: ${c.count} (${c.errors} errors, ${c.warnings} warnings)`); logger.break(); }
      if (result.topRules.length > 0) { logger.log(`  ${highlighter.bold("Top rules:")}`); for (const r of result.topRules) logger.log(`    ${infoSafe(r.rule)}: ${r.count}  [${sanitize(r.category)}]`); logger.break(); }
      if (result.topFiles.length > 0) { logger.log(`  ${highlighter.bold("Top files:")}`); for (const f of result.topFiles) logger.log(`    ${highlighter.warn(String(f.count))}  ${sanitize(f.file)}`); logger.break(); }
