...
import {
  whereUsed,
  buildWhereUsedTree,
  formatWhereUsedAsTree,
  type WhereUsedDirection,
} from "../core/graph.js";
...
export const whereUsedCommand = new Command("where-used")
  ...
  .argument(
    "<component>",
    "component name (Button) or path (src/lib/Button.svelte). Comma-separated for multiple.",
  )
  ...
  .option(
    "--direction <dir>",
    "used-by (who uses it) or uses (what it uses)",
    parseDirection,
    "used-by",
  )
    ...
    (
      ...
      try {
        ...
        if (flags.json) {
          if (flags.tree) {
            const payload = queries.map((query) => ({
              query,
              tree: formatWhereUsedAsTree(
                buildWhereUsedTree(resolvedDirectory, query, flags.scope),
                query,
              ),
            }));
        ...
        for (const query of queries) {
          ...
          logger.log(
            highlighter.bold(
              `  ${result.componentName} (${sanitize(result.componentFile)}) ${directionLabel} ${result.total} ${result.total === 1 ? "place" : "places"}:`,
            ),
          );
          logger.break();
          ...
          if (result.total === 0) {
            ...
            logger.log(
              `  ${result.uniqueFiles} unique file${result.uniqueFiles === 1 ? "" : "s"}, ${result.parentComponents} distinct parent component${result.parentComponents === 1 ? "" : "s"}`,
            );
            logger.break();
          }
        }
