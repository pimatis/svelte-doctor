...
export const initCommand = new Command("init")
  ...
  .action(async (directory: string, flags: { ci?: CliCiPlatform; force?: boolean; yes?: boolean }) => {
    try { await runInit(directory, { ci: flags.ci, force: flags.force ?? false, yes: flags.yes ?? false }); } catch (error) { if (error instanceof Error) logger.error(`  Error: ${error.message}`); process.exit(1); }
  });
