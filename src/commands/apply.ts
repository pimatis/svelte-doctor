  ...
  .action(async (directory: string, flags: Record<string, unknown>) => {
    try {
      ...
      const options: ApplyOptions = {
        dryRun: (flags.dryRun as boolean) ?? false, json: (flags.json as boolean) ?? false,
        write: flags.write === true,
        rules: (flags.rules as string)?.split(",").map((r) => r.trim()).filter(Boolean),
      ...
      if (result.appliedRules.length > 0) { logger.break(); logger.log(`  Applied rules: ${result.appliedRules.join(", ")}`); }
      logger.break();
