...
export const auditCommand = new Command("audit")
  ...
  .action(async (directory: string, flags: { json?: boolean; score?: boolean }) => {
    try {
      ...
      if (flags.score) { logger.log(String(result.securityScore.score)); return; }
      logger.break(); logger.log(`  ${highlighter.bold("svelte-doctor audit")} v${VERSION}`); logger.break();
      ...
      logger.break();
      if (result.securityDiagnostics.length > 0) { for (const d of result.securityDiagnostics) { const icon = d.severity === "error" ? highlighter.error("●") : highlighter.warn("●"); logger.log(`  ${icon} ${sanitize(d.filePath)}:${d.line}  ${sanitize(d.message)}`); logger.dim(`    rule: ${sanitize(d.rule)}  ${sanitize(d.help)}`); } logger.break(); } else { logger.success("  ✓ No security issues found."); logger.break(); }
